...
const RISK_COLORS: Record<string, { bg: string; text: string; border: string; label: string }> = {
    ...
    medium: { bg: '#fef9ec', text: '#9a6700', border: '#f0d88c', label: 'Medium Risk' },
    high: { bg: '#fef2f2', text: '#d93025', border: '#f5c6c6', label: 'High Risk' },
}
...
function ActionPlanView({ plan, onApprove, onApproveAll, onDismiss }: ActionPlanViewProps) {
    const [expandedStep, setExpandedStep] = useState<number | null>(null)
    const [hoveredStep, setHoveredStep] = useState<number | null>(null)

    const hasHighRisk = plan.actions.some((a) => a.risk === 'high')
    const avgConfidence = plan.actions.length > 0
        ? plan.actions.reduce((sum, a) => sum + a.confidence, 0) / plan.actions.length
        : 0
        ...
        <div className="action-plan">
            ...
            <div className="action-plan-footer">
                ...
                <button
                    ...
                    title={hasHighRisk ? 'Cannot auto-run: contains high-risk actions' : 'Run all steps'}
                >
