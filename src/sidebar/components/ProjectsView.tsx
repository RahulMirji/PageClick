...
export function parseUrlPatterns(input: string): string[] {
  return input
    .split("\n")
    .map((p) => p.trim())
    .filter(Boolean);
  ...
  const handleSave = async () => {
    ...
    try {
      const patterns = form.urlPatterns
        ? parseUrlPatterns(form.urlPatterns)
        : [];
      ...
      <div className="projects-view-content">
        {showForm && (
          <div className="project-form">
            ...
            <textarea
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, urlPatterns: e.target.value }))
              }
              rows={3}
            />
            <textarea
        ...
        {projects.length > 0 ? (
                ...
                <div className="project-card-main">
                  ...
                  <div className="project-card-text">
                    ...
                    <span className="project-card-patterns">
                      {p.urlPatterns.length} URL pattern
                      {p.urlPatterns.length !== 1 ? "s" : ""}
                      {p.instructions &&
                        " · " +
                          p.instructions.slice(0, 40) +
                          (p.instructions.length > 40 ? "…" : "")}
                    </span>
                  </div>
                </div>
          ...
          <div className="projects-view-empty">
            ...
            <p>No projects yet</p>
            <span>
              Create a project to give the AI custom instructions per website
            </span>
          </div>
        ) : null}
      </div>
