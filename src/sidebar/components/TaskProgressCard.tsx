...
function TaskProgressCard({ progress }: TaskProgressCardProps) {
    ...
    }, [progress.steps.length, progress.steps[progress.steps.length - 1]?.status])
    ...
    return (
        <div className={`task-progress-card ${allDone ? 'done' : ''} ${hasFailed ? 'has-error' : ''}`} ref={cardRef}>
            {/* Header */}
                    ...
                    <div key={i} className={`task-step task-step-${step.status}`}>
                        <div className="task-step-indicator">
                            ...
                            {i < progress.steps.length - 1 && (
                                <div className={`task-step-line ${step.status === 'completed' ? 'filled' : ''
                                    }`} />
                            )}
                        </div>
