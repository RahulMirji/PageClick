...
export class TaskOrchestrator {
    ...
    isActive(): boolean {
        return this.state.phase !== 'idle' && this.state.phase !== 'completed' && this.state.phase !== 'error'
    }
    ...
    startTask(goal: string): void {
        ...
        this.emit({ type: 'PHASE_CHANGE', phase: 'clarifying', message: this.state.statusMessage })
    }
    ...
    beginExecution(): void {
        ...
        this.emit({ type: 'PHASE_CHANGE', phase: 'executing', message: this.state.statusMessage })
    }
    ...
    completeLoop(entry: LoopEntry): boolean {
        ...
        this.emit({ type: 'PHASE_CHANGE', phase: 'observing', message: this.state.statusMessage })
        return true
    }
    ...
    abort(reason?: string): void {
        ...
        this.emit({ type: 'PHASE_CHANGE', phase: 'idle', message: this.state.statusMessage })
    }
    ...
    buildHistorySummary(): string {
        ...
        for (const entry of this.state.history) {
            ...
            for (const r of entry.results) {
                ...
                lines.push(`  ${status} ${r.action} → ${r.selector}${r.error ? ` (Error: ${r.error})` : ''}${r.extractedData ? ` [Extracted: ${r.extractedData}]` : ''}`)
            }
        }
        return lines.join('\n')
    }
