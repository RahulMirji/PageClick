    ...
    try {
        ...
        stopFn = () => {
            chrome.scripting.executeScript({
                target: { tabId },
                func: () => {
                    const canvas = document.getElementById('__pc-vignette') as HTMLCanvasElement | null
...
function injectVignetteOverlay() {
    ...
    if (!ctx) { canvas.remove(); pill.remove(); style.remove(); return }
    ctx.scale(dpr, dpr)
    ...
    function renderVignette(breathAlpha: number, sweepAngle: number) {
        ...
        for (const c of corners) {
            const cGrad = ctx!.createRadialGradient(c.x, c.y, 0, c.x, c.y, cornerRadius)
            cGrad.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${cornerAlpha})`)
            cGrad.addColorStop(1, `rgba(${r}, ${g}, ${b}, 0)`)
            ctx!.fillStyle = cGrad
            ctx!.fillRect(
                c.x - cornerRadius, c.y - cornerRadius,
        ...
        if (sweepPos < W) {
            ...
            sx = W - (sweepPos - W - H); sy = H
        } else {
            sx = 0; sy = H - (sweepPos - 2 * W - H)
        }
        ...
        ctx!.fillRect(sx - sweepRadius, sy - sweepRadius, sweepRadius * 2, sweepRadius * 2)

        // Second sweep (offset by half)
        const sweep2Pos = ((sweepAngle + Math.PI) % (Math.PI * 2)) / (Math.PI * 2) * perimeter
        let sx2: number, sy2: number

        if (sweep2Pos < W) {
            sx2 = sweep2Pos; sy2 = 0
        } else if (sweep2Pos < W + H) {
            sx2 = W; sy2 = sweep2Pos - W
        } else if (sweep2Pos < 2 * W + H) {
            sx2 = W - (sweep2Pos - W - H); sy2 = H
        } else {
            sx2 = 0; sy2 = H - (sweep2Pos - 2 * W - H)
        }

        const sweep2Grad = ctx!.createRadialGradient(sx2, sy2, 0, sx2, sy2, sweepRadius * 0.8)
        ...
        ctx!.fillRect(sx2 - sweepRadius, sy2 - sweepRadius, sweepRadius * 2, sweepRadius * 2)
    }
