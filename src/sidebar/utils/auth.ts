...
function mapSupabaseUser(su: SupabaseUser): User {
    ...
    const name = meta.full_name || meta.name || idData.full_name || idData.name || meta.preferred_username || su.email?.split('@')[0] || 'User'
...
export async function signInWithGoogle(): Promise<User> {
    if (!GOOGLE_CLIENT_ID) {
        throw new Error('Google Client ID not configured. Check VITE_GOOGLE_CLIENT_ID in .env')
    }
