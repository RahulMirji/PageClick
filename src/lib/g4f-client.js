...
function captureUserTierHeaders(headers, usage) {
  ...
  const modelFactor = parseFloat(
    headers.get("x-ratelimit-model-factor") || "1",
  );
  const remainingRequests =
    parseInt(headers.get("x-ratelimit-remaining-requests") || "1") -
    (usage ? 1 : 0);
  let totalTokens =
    usage?.total_tokens || parseInt(headers.get("x-usage-total-tokens") || "0");
  let remainingTokens = parseInt(
  ...
  if (
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("userTierUpdate", { detail: userInfo }),
      );
    }
  }
}
...
class Client {
  ...
  async _fetchWithProxyRotation(targetUrl, requestConfig = {}) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        console.warn(
          `CORS proxy attempt ${attempt + 1}/${maxAttempts} failed for ${targetUrl}:`,
          error.message,
        );
  ...
  async _sleep() {
    if (this.sleep && this.lastRequest) {
      ...
      while (this.sleep > timeSinceLastRequest) {
        console.log(
          `Sleeping for ${this.sleep - timeSinceLastRequest} ms to respect rate limits.`,
        );
  ...
  async _defaultImageGeneration(imageEndpoint, params, requestOptions) {
    ...
    if (params.response_format === "b64_json") {
      ...
      return {
        data: [
          { b64_json: await toBase64(data).then((b64) => b64.split(",")[1]) },
        ],
      };
    }
...
class PollinationsAI extends Client {
  constructor(options = {}) {
    super({
      ...
      modelsEndpoint:
        options.modelsEndpoint || "https://gen.pollinations.ai/text/models",
      imageModelsEndpoint:
        options.imageModelsEndpoint || options.apiKey
        ...
        try {
          ...
          try {
            ...
            if (!textModelsResponse.ok) {
              throw new Error(
                `Status ${textModelsResponse.status}: ${await textModelsResponse.text()}`,
              );
            }
          } catch (e) {
            console.error(
          ...
          try {
            ...
            if (!imageModelsResponse.ok) {
              ...
              if (!imageModelsResponse.ok) {
                throw new Error(
                  `Status ${imageModelsResponse.status}: ${await imageModelsResponse.text()}`,
                );
              }
            }
          } catch (e) {
