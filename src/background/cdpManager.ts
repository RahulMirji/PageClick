...
import type { CDPNetworkEntry, CDPConsoleEntry, CDPSnapshot } from '../shared/messages'
...
function sendCommand(tabId: number, method: string, params: object = {}): Promise<any> {
...
class CDPManager {
    ...
    async evalJs(tabId: number, expression: string): Promise<{ result?: string; error?: string }> {
        if (!this.sessions.has(tabId)) {
        ...
        try {
            ...
            if (res?.exceptionDetails) {
                return { error: res.exceptionDetails.text || 'Eval threw an exception' }
            }
    ...
    private onEvent(
        ...
        switch (method) {
            ...
            case 'Network.responseReceived': {
                ...
                const encodedDataLength: number = params.response?.encodedDataLength ?? Infinity
            ...
            case 'Runtime.exceptionThrown': {
                ...
                const text = detail?.exception?.description
                    ?? detail?.text
                    ?? 'Unknown JS error'
