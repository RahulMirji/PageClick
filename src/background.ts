...
const NATIVE_HOST_NAME = "com.pageclick.host";
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  console.log(
    "%c[PageClick:BG] Message received:",
  ...
  if (message.type === "EXECUTE_ACTION") {
    ...
    console.log("%c[PageClick:BG] EXECUTE_ACTION:", "color: #a78bfa", {
      action: step.action,
    ...
    if (step.action === "navigate") {
      console.log(
        "%c[PageClick:BG] Routing navigate to background handler",
        "color: #a78bfa",
      );
      handleNavigateAction(step, sendResponse);
    } else {
      console.log(
        "%c[PageClick:BG] Forwarding to content script",
        "color: #a78bfa",
      );
  ...
  // NATIVE_HOST_CALL — route a validated native operation to local host app
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          sendResponse({
            ok: false,
            error:
              chrome.runtime.lastError.message || "Native host request failed",
          });
          return;
        }
        sendResponse(
          response ?? { ok: false, error: "Empty native host response" },
        );
      },
    );
    return true;
    ...
    (async () => {
      try {
        ...
        const patterns = (urls || []).map(
          (u: string) => new RegExp(u.replace(/\*/g, ".*"), "i"),
        );
        const matchingIds = allTabs
          .filter(
            (t) =>
              t.id &&
              patterns.some(
                (p: RegExp) => p.test(t.url || "") || p.test(t.title || ""),
              ),
          )
          .map((t) => t.id!);
        ...
        sendResponse({
          ok: false,
          error: err.message || "Failed to create tab group",
        });
      }
    })();
    return true;
    ...
    (async () => {
      try {
        ...
        if (groups.length === 0) {
          sendResponse({
            ok: false,
            error: `No tab group found with title "${title}"`,
          });
          return;
        }
        ...
        if (matchingIds.length === 0) {
          sendResponse({
            ok: false,
            error: "No ungrouped tabs matched the given patterns",
          });
          return;
        }
        ...
        sendResponse({
          ok: false,
          error: err.message || "Failed to add tabs to group",
        });
      }
    })();
    return true;
  ...
  if (message.type === "TAB_GROUP_LIST") {
    (async () => {
      try {
        ...
        sendResponse({
          ok: false,
          error: err.message || "Failed to list tab groups",
        });
      }
    })();
    return true;
...
async function forwardToActiveTab(
  ...
  try {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    const tab = tabs[0];
    console.log("%c[PageClick:BG] forwardToActiveTab:", "color: #a78bfa", {
      tabId: tab?.id,
      tabUrl: tab?.url,
    ...
    if (isRestricted) {
      ...
      console.warn(
        "%c[PageClick:BG] BLOCKED: Cannot execute on restricted page:",
        "color: #ef4444",
        url,
      );
      sendResponse({
        ...
        result: {
          ...
          error:
            "Cannot execute actions on this page. Try navigating to a regular webpage first.",
          durationMs: 0,
        },
      });
      return;
    ...
    if (!isRestricted) {
      console.log(
        "%c[PageClick:BG] Injecting content script...",
        "color: #a78bfa",
      );
      try {
        await chrome.scripting.executeScript({
        ...
        console.log(
          "%c[PageClick:BG] Content script injected (or already present)",
          "color: #a78bfa",
        );
      } catch (injectErr) {
        console.warn(
          "%c[PageClick:BG] Content script injection failed:",
          "color: #f59e0b",
          injectErr,
        );
      }
    ...
    console.log(
      "%c[PageClick:BG] Response from tab:",
      "color: #a78bfa",
      response,
    );
    sendResponse(response);
    ...
    console.error(
      "%c[PageClick:BG] forwardToActiveTab FAILED:",
...
async function handleNavigateAction(
  ...
  console.log(
    "%c[PageClick:BG] handleNavigateAction called:",
  ...
  try {
    ...
    if (!tab?.id) {
      console.warn(
        "%c[PageClick:BG] Navigate: No active tab!",
        "color: #ef4444",
      );
      sendResponse({
        type: "EXECUTE_ACTION_RESULT",
    ...
    console.log(
      "%c[PageClick:BG] Navigate target URL raw:",
      "color: #60a5fa",
      targetUrl,
    );
    if (!targetUrl) {
      console.warn(
        "%c[PageClick:BG] Navigate: No URL provided!",
        "color: #ef4444",
      );
      sendResponse({
        type: "EXECUTE_ACTION_RESULT",
    ...
    console.error(
      "%c[PageClick:BG] Navigate FAILED:",
      "color: #ef4444; font-weight: bold",
      err,
    );
    sendResponse({
