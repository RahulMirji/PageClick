...
function findElement(selector: string): Element | null {
    try {
        ...
        console.log('%c[PageClick:CS] findElement:', 'color: #22d3ee', { selector, found: !!el, tagName: el?.tagName, id: (el as any)?.id })
        return el
    } catch (e) {
        console.warn('%c[PageClick:CS] Invalid selector:', 'color: #ef4444', selector, e)
        return null
    }
}
...
export async function executeAction(step: ActionStep): Promise<ExecutionResult> {
    ...
    console.log('%c[PageClick:CS] ┌── executeAction called', 'color: #22d3ee; font-weight: bold', {
        action: step.action,
    ...
    if (!el) {
        console.warn('%c[PageClick:CS] └── Element NOT FOUND:', 'color: #ef4444; font-weight: bold', step.selector)
        return {
            success: false,
            action: step.action,
            selector: step.selector,
    ...
    try {
        switch (step.action) {
            ...
            case 'input':
                if (!step.value) {
                    console.warn('%c[PageClick:CS] └── Input requires value!', 'color: #ef4444')
                    return {
                        success: false, action: step.action, selector: step.selector,
                        error: 'Input action requires a value', durationMs: performance.now() - start,
                    }
                }
                console.log('%c[PageClick:CS] │ Executing INPUT:', 'color: #22d3ee', step.value)
                await executeInput(el, step.value)
                flashElement(el)
                break

            case 'scroll':
                console.log('%c[PageClick:CS] │ Executing SCROLL:', 'color: #22d3ee', step.value || 'down')
            ...
            case 'extract':
                ...
                console.log('%c[PageClick:CS] └── Extracted:', 'color: #34d399; font-weight: bold', data?.slice(0, 200))
                flashElement(el)
                return {
                    success: true, action: step.action, selector: step.selector,
                    extractedData: data, durationMs: performance.now() - start,
                }
            ...
            default:
                console.warn('%c[PageClick:CS] └── Unknown action:', 'color: #ef4444', step.action)
                return {
                    success: false, action: step.action, selector: step.selector,
                    error: `Unknown action: ${step.action}`, durationMs: performance.now() - start,
                }
        }

        // Apply wait strategy
        console.log('%c[PageClick:CS] │ Applying wait strategy:', 'color: #22d3ee', step.waitFor || 'default (150ms)')
        await applyWaitStrategy(step.waitFor, step.timeoutMs)

        const duration = performance.now() - start
        console.log('%c[PageClick:CS] └── SUCCESS in', 'color: #34d399; font-weight: bold', `${duration.toFixed(0)}ms`)
        return {
            success: true, action: step.action, selector: step.selector,
            durationMs: duration,
        }
    } catch (err: any) {
        console.error('%c[PageClick:CS] └── FAILED:', 'color: #ef4444; font-weight: bold', err)
        flashError(el)
        return {
            success: false, action: step.action, selector: step.selector,
